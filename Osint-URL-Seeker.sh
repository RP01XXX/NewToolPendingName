...
# DNS Recon Run
echo "[6] DNS Recon is running.........."
dnsrecon -d $url -a -c $path/$Name/

# DIG
echo "[7] DIG is digging......."
dig $url > $path/$Name/digResults.txt

# The Harvester
echo "[8] TheHarvester is running............."
theHarvester -d <URL> -l 200 -b bing,bingapi,certspotter,dnsdumpster,sublist3r,urlscan,virustotal -f $path/$Name/Harvested.txt

echo "Enter Your Wordlist for Subdomain Brute Forcing"
read wordlist

echo "[9] Brute Forcing Subdomains........"
knock.py $url -w $wordlist -o $path/$url

echo "[10] Running Nikto..............."
nikto -host $url

echo "[11] Curl options ......."
curl -X options $url

echo "[12] Is it Wordpress?..........."
wpscan --url $url --wp-content-dir /wp-content/ --enumerate vp --plugins-detection aggressive --enumerate u -o "$path/$Name/WordpressScan.txt"

echo "[Final] Is there a WAF?......"
wafw00f $url ?? $path/$Name/WAF.txt

echo "Cleaning up your results"

echo "[8/] identify unique IP's from  Amass"
...
echo "[9/] identify unique IP's from  NSLOOKUP"
